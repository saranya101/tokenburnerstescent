...
describe.skipIf(!testDatabaseUrl)("Prisma PostgreSQL restart safety", () => {
  let db: PrismaClient; let executionId = ""; let approvalId = "";
  beforeAll(async () => { db = new PrismaClient({ datasourceUrl: testDatabaseUrl! }); await cleanup(db); await db.user.create({ data: { id: ids.user } }); await db.account.createMany({ data: [{ id: "it-db-acc-sgd", userId: ids.user, providerRef: "acc-sgd", type: "CHECKING", currency: "SGD", ledgerMinorUnits: 2_000_000n, availableMinorUnits: 2_000_000n, status: "ACTIVE", capabilities: ["SEND_TRANSFER", "CONVERT_FX"] }, { id: "it-db-acc-usd", userId: ids.user, providerRef: "acc-usd", type: "CHECKING", currency: "USD", ledgerMinorUnits: 500_000n, availableMinorUnits: 500_000n, status: "ACTIVE", capabilities: ["SEND_TRANSFER"] }] }); await db.beneficiary.create({ data: { id: "it-db-ben-ntu", userId: ids.user, providerRef: "ben-ntu", name: "NTU", supportedCurrencies: ["USD"], status: "ACTIVE" } });
    const fixtureGoal = GoalContractV1.parse(fixture("goal-contract.json")); const unhashed = GoalContractV1.parse({ ...fixtureGoal, id: ids.goal, userId: ids.user, sourceIntentDraftId: undefined }); const goal = { ...unhashed, contractHash: hashGoalContract(unhashed) };
    await db.goalContract.create({ data: { id: ids.goalRow, contractKey: goal.id, version: goal.version, userId: goal.userId, status: "CONFIRMED", schemaVersion: goal.schemaVersion, goalPayload: goal.goal, preferences: goal.preferences, contractHash: goal.contractHash, createdAt: new Date(goal.createdAt), confirmedAt: new Date(goal.confirmedAt!), constraints: { create: goal.constraints.map(({ type, ...payload }) => ({ type, payload })) }, entityBindings: { create: goal.entityBindings.map((binding) => ({ reference: binding.reference, entityType: binding.entityType, entityId: binding.entityId, resolutionMethod: binding.resolutionMethod, ...(binding.confidence === undefined ? {} : { confidence: binding.confidence }), confirmed: binding.confirmed })) } } }); });
  ...
  it("persists the NTU pipeline, rolls transactions back, enforces idempotency, and reconstructs after reconnect", async () => {
    const repository = new PrismaParlanceRepository(db); const bank = await mockBank(); const compiler = { async compile() { const plan = FinancialPlanV1.parse({ ...(fixture("financial-plan.json") as object), id: "it-plan-ntu", goalContractId: ids.goal }); return CompilerResultV1.parse({ schemaVersion: "1", status: "SAT", plan }); } };

...
export class CompilerClient {
  ...
  async compile(goal: GoalContractV1, state: BankStateSnapshotV1) {
    const response = await fetch(`${this.baseUrl}/v1/compile`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ goalContract: goal, bankStateSnapshot: state }) });
